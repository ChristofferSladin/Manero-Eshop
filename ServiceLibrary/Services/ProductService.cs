...
public class ProductService : IProductService
{
    ...
    public async Task<List<Product>> SelectFilteredProductsAsync(
    int? page, int? take,
    string? category, string? orderBy, string? orderDirection,
    string? filterByName, string? gender,
    string? color, decimal? minPrice, decimal? maxPrice, List<string> sizes)
    {
        var products = new List<Product>();
        try
        {
            ...
            if (!string.IsNullOrEmpty(color))
                query.Append($"color={Uri.EscapeDataString(color)}&");
            ...
            if (sizes != null && sizes.Any())
            {
                ...
                {
                    query.Append($"sizes={Uri.EscapeDataString(size)}&");
                }
            }
